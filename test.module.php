	...
	if ($_GET['action'] =='add_theme')
		{
			...
			{
				$sql = "INSERT INTO {$mod_table_th} VALUES (
					'NULL','".$modx->db->escape($_POST['theme_title'])."',
					'".$modx->db->escape($_POST['theme_description'])."',
					'".$modx->db->escape($_POST['test_type'])."',
					'".$modx->db->escape($_POST['test_time'])."')";
				//echo $sql;
				$modx->db->query($sql);
